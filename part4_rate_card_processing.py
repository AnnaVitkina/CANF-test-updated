...
def _load_combined_rate_card(full_path):
    """
    Load a pre-combined rate card file (rate_card_modified.xlsx).
    
    This file was created by multiple_rates.py and already has processed data.
    
    Args:
        full_path (str): Full path to the combined rate card file
    
    Returns:
        tuple: (dataframe, list of column names, conditions dictionary)
    """
    print(f"\n[INFO] Loading pre-combined rate card: {full_path}")
    
    # Read the data sheet
    df = pd.read_excel(full_path, sheet_name='Rate Card Data')
    ...
    conditions = {}
    try:
        df_conditions = pd.read_excel(full_path, sheet_name='Conditions')
        for _, row in df_conditions.iterrows():
            if row.get('Has Condition') == 'Yes' and pd.notna(row.get('Condition Rule')):
                conditions[row['Column']] = row['Condition Rule']
    except Exception as e:
        print(f"   [WARNING] Could not read conditions sheet: {e}")
...
def _load_combined_business_rules(full_path):
    """
    Load business rules from a pre-combined rate card file (rate_card_modified.xlsx).
    
    Args:
        full_path (str): Full path to the combined rate card file
    
    Returns:
        dict: Business rules dictionary
    """
    print(f"\n[INFO] Loading business rules from pre-combined file: {full_path}")
    ...
    try:
        df_rules = pd.read_excel(full_path, sheet_name='Business Rules')
        
        for _, row in df_rules.iterrows():
            rule_data = {
                'name': row.get('Rule Name'),
                'section': row.get('Section', '').lower().replace(' ', '_'),
                'country': row.get('Country'),
                'postal_code': row.get('Postal Codes'),
                'exclude': row.get('Exclude') == 'Yes'
            }
            
            result['raw_rules'].append(rule_data)
            
            section = rule_data['section']
            if 'postal' in section:
                result['postal_code_zones'].append(rule_data)
            elif 'country' in section:
                result['country_regions'].append(rule_data)
            elif 'no_data' in section:
                result['no_data_added'].append(rule_data)
        
        print(f"   Loaded {len(result['raw_rules'])} business rules")
        
    except Exception as e:
        print(f"   [WARNING] Could not read Business Rules sheet: {e}")
...
def process_rate_card(file_path):
    ...
    # Check if this is a pre-combined rate card file (rate_card_modified.xlsx)
    # These files have 'Rate Card Data' and 'Conditions' sheets already processed
    if 'rate_card_modified' in file_path.lower():
        return _load_combined_rate_card(full_path)
...
def process_business_rules(file_path):
    ...
    # Check if this is a pre-combined rate card file
    if 'rate_card_modified' in file_path.lower():
        return _load_combined_business_rules(full_path)
